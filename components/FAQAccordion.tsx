      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="rounded-xl border border-neutral-100 bg-white overflow-hidden">
            ...
            >
              <span className="text-sm font-medium text-neutral-800 pr-4">{item.question}</span>
              <span className="shrink-0 text-neutral-400 text-lg leading-none font-light">
                {open === i ? "−" : "+"}
              </span>
            </button>
            {open === i && (
              <div className="px-5 pb-4 text-sm leading-relaxed border-t border-neutral-50"
