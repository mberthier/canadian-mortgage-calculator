    ...
    <div className="border-t border-slate-100 pt-5">
      <button onClick={() => setOpen((o) => !o)}
        ...
        <div className="flex items-center gap-2">
          {ltt.net > 0 && <span className="normal-case font-normal text-slate-500">{formatCurrency(ltt.net, 0)}</span>}
      ...
      {open && (
        <div className="mt-4 space-y-3">
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-slate-700">First-time home buyer</span>
          </label>

          <div className="rounded-lg border border-slate-100 p-3 space-y-2 text-sm" style={{ background: "var(--cream)" }}>
            {ltt.provincial > 0 && (
            ...
            {ltt.net === 0 && ["SK","NL","NT","NU","YT"].includes(inputs.province) && (
              <p className="text-xs text-slate-400">No provincial land transfer tax in this province.</p>
            )}
          </div>
        </div>
      )}
