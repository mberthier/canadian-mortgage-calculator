...
function Step({ number, title, children }: StepProps) {
  return (
    <div className="flex gap-4">
      ...
      <div className="flex-1 pb-5 border-b border-stone-100 last:border-0 last:pb-0">
        <p className="text-sm font-semibold text-stone-800 mb-1.5">{title}</p>
        {children}
      </div>
    </div>
  );
      ...
      {open && (
        <div className="mt-4 space-y-5">
          ...
          <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border"
            ...
            <div>
              <p className="text-sm font-semibold text-stone-800">I'm a first-time home buyer</p>
          ...
          <div className="space-y-0">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr style={{ background: "var(--cream)" }}>
                        <th className="px-3 py-2 text-left font-semibold text-stone-600">Purchase Price</th>
                        <th className="px-3 py-2 text-right font-semibold text-stone-600">Min. Down</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr className={homePrice <= 500_000 ? "bg-green-50" : ""}>
                        <td className="px-3 py-2 text-stone-600">Under $500,000</td>
                        <td className="px-3 py-2 text-right font-medium text-stone-800">5%</td>
                      </tr>
                      <tr className={homePrice > 500_000 && homePrice <= 1_500_000 ? "bg-green-50" : ""}>
                        <td className="px-3 py-2 text-stone-600">$500K – $1.5M</td>
                        <td className="px-3 py-2 text-right font-medium text-stone-800">5% + 10% over $500K</td>
                      </tr>
                      <tr className={homePrice > 1_500_000 ? "bg-green-50" : ""}>
                        <td className="px-3 py-2 text-stone-600">Over $1.5M</td>
                        <td className="px-3 py-2 text-right font-medium text-stone-800">20%</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <Step number={2} title="What is CMHC insurance and do you need it?">
              <div className="space-y-2 text-xs" style={{ color: "var(--ink-mid)" }}>
                <p>
                  If your down payment is <strong className="text-stone-700">less than 20%</strong>, the federal government requires you to buy mortgage default insurance through CMHC. The premium is added to your mortgage — you don't pay it upfront.
                </p>
                <div className="rounded-lg overflow-hidden border border-stone-100">
                  <table className="w-full text-xs">
                    <thead>
                      <tr style={{ background: "var(--cream)" }}>
                        <th className="px-3 py-2 text-left font-semibold text-stone-600">Down Payment</th>
                        <th className="px-3 py-2 text-right font-semibold text-stone-600">Premium</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ...
                        <tr key={range} style={active ? { background: "var(--green-light)" } : {}}>
                          ...
                          <td className="px-3 py-2 text-right font-medium text-stone-800">{rate}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <Step number={3} title="What is the mortgage stress test?">
              <div className="space-y-2 text-xs" style={{ color: "var(--ink-mid)" }}>
                <p>
                  Before approving your mortgage, your lender must verify you can afford payments at a higher rate — either your contract rate <strong className="text-stone-700">+ 2%</strong> or <strong className="text-stone-700">5.25%</strong>, whichever is higher.
                </p>
                <p>This protects you from defaulting if rates rise at renewal.</p>
                {interestRate > 0 && (
                  <div className="rounded-lg px-3 py-2 border border-stone-100"
                    style={{ background: "var(--cream)" }}>
                    At your rate of <strong className="text-stone-700">{interestRate}%</strong>, you'll be stress-tested at{" "}
            ...
            <Step number={4} title="Land transfer tax — and your rebate">
              <div className="space-y-2 text-xs" style={{ color: "var(--ink-mid)" }}>
                <p>
                  When you buy a home, you pay a <strong className="text-stone-700">land transfer tax</strong> to your province (and Toronto charges an additional municipal LTT).
                </p>
                {homePrice > 0 && (
                  <div className="rounded-lg overflow-hidden border border-stone-100">
                    <table className="w-full text-xs">
                      <tbody>
                        <tr>
                          <td className="px-3 py-2 text-stone-600">LTT in {PROVINCE_NAMES[province] ?? province}</td>
                        ...
                        {hasRebate && (
                          <>
                            ...
                            <tr style={{ background: "var(--green-light)" }}>
                              <td className="px-3 py-2 font-semibold text-stone-700">Your net LTT</td>
                        ...
                        {!hasRebate && (
                          <tr>
                            <td className="px-3 py-2 text-stone-400" colSpan={2}>
            ...
            <Step number={5} title="The Home Buyers' Plan — use your RRSP">
              <div className="text-xs space-y-2" style={{ color: "var(--ink-mid)" }}>
                <p>
                  First-time buyers can withdraw up to <strong className="text-stone-700">$60,000 per person</strong> from their RRSP tax-free to use toward a down payment (increased from $35K in 2024).
                </p>
                <p>
                  Couples can withdraw up to <strong className="text-stone-700">$120,000 combined</strong>. You have 15 years to repay the withdrawn amount back into your RRSP.
                </p>
                <div className="rounded-lg px-3 py-2 border"
                  style={{ background: "var(--green-light)", borderColor: "var(--green-border)", color: "var(--green)" }}>
