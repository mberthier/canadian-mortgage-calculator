...
export default function MortgageComparison({ inputs, loanAmount }: Props) {
  ...
  const sel = "w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-700/20 focus:border-blue-600 transition-colors appearance-none cursor-pointer";
    ...
    <div ref={sectionRef} className="rounded-2xl bg-white border border-slate-100 overflow-hidden">
      <button onClick={() => setOpen(o => !o)}
        ...
        <div className="flex items-center gap-2">
          <p className="text-sm font-semibold text-slate-800">What if you got a better rate?</p>
        ...
        <div className="px-5 pb-5 border-t border-slate-100 pt-4 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl p-4 border-2" style={{ borderColor: "var(--green)", background: "var(--green-light)" }}>
              ...
              <p className="text-base font-semibold text-slate-900">{inputs.interestRate.toFixed(2)}%</p>
              <p className="text-xs text-slate-500 mb-0.5 mt-2">Amortization</p>
              <p className="text-base font-semibold text-slate-900">{inputs.amortizationYears} years</p>
            </div>

            <div className="rounded-xl p-4 border border-slate-200">
              <p className="text-xs font-bold uppercase tracking-widest mb-3 text-slate-500">Scenario B</p>
              <div className="space-y-2">
                ...
                <div>
                  <label className="text-xs text-slate-500">Amortization</label>
                  <div className="relative mt-0.5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--cream)" }}>
                  ...
                  <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">B</th>
                  <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-500 uppercase tracking-wide">Diff</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 text-xs">
                <tr>
                  <td className="px-4 py-2.5 font-medium text-slate-600">Payment</td>
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-700 tabular-nums">{formatCurrency(scenarioB.payment, 2)}</td>
                ...
                <tr style={{ background: "var(--cream)" }}>
                  <td className="px-4 py-2.5 font-semibold text-slate-700" colSpan={4}>After {inputs.termYears}-year term</td>
                </tr>
                <tr>
                  <td className="px-4 py-2.5 text-slate-600">Interest paid</td>
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-700 tabular-nums">{formatCurrency(scenarioB.termInterest, 0)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-medium"
                ...
                <tr>
                  <td className="px-4 py-2.5 text-slate-600">Balance at renewal</td>
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-700 tabular-nums">{formatCurrency(scenarioB.termEndBalance, 0, true)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-medium"
                ...
                <tr style={{ background: "var(--cream)" }}>
                  <td className="px-4 py-2.5 font-semibold text-slate-700" colSpan={4}>Full amortization</td>
                </tr>
                <tr>
                  <td className="px-4 py-2.5 text-slate-600">Total interest</td>
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-700 tabular-nums">{formatCurrency(scenarioB.totalInterest, 0, true)}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums font-medium"
                ...
                <tr>
                  <td className="px-4 py-2.5 text-slate-600">Paid off in</td>
                  <td className="px-4 py-2.5 text-right font-semibold" style={{ color: "var(--green)" }}>{scenarioA.actualAmortYears.toFixed(1)} yrs</td>
                  <td className="px-4 py-2.5 text-right text-slate-700">{scenarioB.actualAmortYears.toFixed(1)} yrs</td>
                  <td className="px-4 py-2.5 text-right text-xs font-medium text-slate-400">
