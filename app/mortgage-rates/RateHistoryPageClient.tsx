...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-xl px-3 py-2.5 shadow-sm text-xs">
      ...
      {point?.event && (
        <p className="mt-1.5 pt-1.5 border-t border-stone-100 text-stone-500 font-medium">{point.event}</p>
      )}
    </div>
  );
};
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="rounded-2xl bg-white border border-stone-100 p-4 text-center shadow-sm">
      ...
      <div className="rounded-2xl bg-white border border-stone-100 shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-semibold text-stone-700">Rate History</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="label" tickLine={false} axisLine={false}
                tick={{ fontSize: 11, fill: "#a8a29e" }} interval={tickInterval} />
      ...
      {/* Key events timeline */}
      <div className="rounded-2xl bg-white border border-stone-100 shadow-sm p-5">
        <p className="text-sm font-semibold text-stone-700 mb-4">Key Rate Events</p>
          ...
          {RATE_HISTORY.filter((p) => p.event && p.date >= cutoff).map((p) => (
            <div key={p.date} className="flex gap-4 items-start">
              ...
              <div className="flex items-center gap-2 flex-1">
                ...
                <span className="text-xs text-stone-600">{p.event}</span>
