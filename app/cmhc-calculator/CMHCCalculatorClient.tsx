    ...
    <div className="rounded-2xl bg-white border border-stone-100 shadow-sm overflow-hidden">
      <div className="p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-stone-500 mb-1.5 uppercase tracking-wide">Home Price</label>
          ...
          <div>
            <label className="block text-xs font-medium text-stone-500 mb-1.5 uppercase tracking-wide">Interest Rate</label>
            <div className="relative">
      ...
      {/* Results */}
      <div className="border-t border-stone-100 p-6" style={{ background: hasCMHC ? "var(--cream)" : "var(--green-light)" }}>
        {hasCMHC ? (
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-white border border-stone-100 p-4">
              <p className="text-xs font-semibold text-stone-600 mb-3">Payment comparison: {downPct.toFixed(1)}% down vs 20% down</p>
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  <p className="text-2xl font-semibold" style={{ color: "var(--amber)" }}>{formatCurrency(paymentWith, 2)}</p>
                  <p className="text-xs text-stone-500 mt-0.5">With CMHC ({downPct.toFixed(1)}% down)</p>
                </div>
                <div>
                  <p className="text-2xl font-semibold" style={{ color: "var(--green)" }}>{formatCurrency(paymentWithout, 2)}</p>
                  <p className="text-xs text-stone-500 mt-0.5">Without CMHC (20% down)</p>
                </div>
              </div>
              <p className="text-xs text-center mt-3" style={{ color: "var(--ink-faint)" }}>
                Monthly saving by reaching 20% down: <span className="font-semibold text-stone-700">{formatCurrency(paymentWith - paymentWithout, 2)}</span>
              </p>
            </div>
          </div>
        ) : (
          <div className="text-center py-4">
            ...
            <p className="text-base font-semibold mt-3 text-stone-800">
